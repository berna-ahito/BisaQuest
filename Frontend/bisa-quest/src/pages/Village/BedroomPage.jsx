...
import { NPC_IMAGES } from "./data/bedroomData";
import {
  SCENE_QUEST_IDS,
  fetchSceneItems,
  fetchSceneDialogues,
...
const BedroomPage = () => {
  ...
  useEffect(() => {
    const loadSceneData = async () => {
      const questId = SCENE_QUEST_IDS.bedroom;
  ...
  const handleQuestComplete = async (region) => {
    ...
    saveNPCProgress("village", "village_bedroom", next.size, passed, 3, [word]);

    if (playerId && location.state?.questId) {
      submitChallenge(playerId, location.state.questId, npcId, next.size, 3, passed)
        .catch(err => console.error("[BedroomPage] submitChallenge failed:", err));
    }
